<template>
  <div class="question-file-content">
    <section class="question-file-content-inner">
      <section v-if="!hideTitle" class="question-file-header">
        <div>{{ $t('workOrderInfo.descProT') }}</div>
        <section>
      ...
      <div class="problem-description-form">
        <el-form
          ...
          <el-form-item :label="$t(`workOrderInfo.Merchant`)" prop="merchantId">
            <!-- <el-select
              v-model="formLabel.merchantId"
              :placeholder="$t(`workOrderInfo.enterTheMerchantNumber`)"
              filterable
              clearable>
              <el-option
...
import { orderCreate, getRecentMerchantList } from '@/api/workOrder';
...
export default {
  ...
  created() {
    this.getRecentMerchantListMess();
  },
  mounted() {
  ...
  methods: {
        ...
        .then((response) => {
          ...
          if (code === 'APPLY_SUCCESS') {
            data.map((v) => {
              newArry.push({
                value: v.merchantId,
                address: v.merchantId
              });
            });
            this.restaurants = newArry;
            if (newArry && newArry.length) {
              this.formLabel.merchantId = newArry[0].value;
            }
          }
        }).catch((error) => {
          console.log(error);
    ...
    getRealmerchantId(val) {
      let real = val;
      (this.restaurants || []).forEach((test) => {
        if (val.trim() == test.value.trim()) {
          real = test.address;
        }
      });
      return real;
    ...
    onSubmit() {
      if (this.formLabel.desc) {
        ...
        const params = {
          ...
          'type': this.$route.query.type || 'CONSULT',
          ...
          'merchantId': this.getRealmerchantId(this.formLabel.merchantId) != '' ? this.getRealmerchantId(this.formLabel.merchantId).trim() : '',
          source
        };
          ...
          .then((response) => {
            ...
            switch (code) {
              case 'WORK_ORDER_ALREADY_EXISTS':
                ...
                break;
              case 'MERCHANT_NOT_EXITS':
                this.tipMessage(this.$t('workOrderInfo.Pmnum'));
                break;
              default:
                break;
            }
