  ...
  <div class="work-order">
    <full-screen-layout :show-has-back-btn="false" content-width="100%">
      ...
      <template slot="content">
        <Search v-if="showSearch && source == 0" @searchContent="searchContent" />
...
export default {
  ...
  data() {
    return {
      ...
      timerIdTwo: null,
      source: parseInt(window.localStorage.getItem('platform-work-order')) || 0,
    };
  },
  computed: {
  ...
  methods: {
    ...
    queryNoticeCountTotal() {
      getUnreadMessageList({ pageSize: 20, pageNum: 1 })
        .then(response => {
          if (response.data) {
